...
# test data
test_sample_idx = np.random.randint(0, scaled_test_data.shape[0],
                                    size=5)
X_test_sample = scaled_test_data[test_sample_idx, :]
...
#%% occluded test data
X_occl_sample = X_test_sample.view(-1, 28, 28)
for i in range(X_occl_sample.shape[0]):
    startloc = np.random.randint(10, 21)
    if i % 2 == 0:
        X_occl_sample[i, startloc:startloc+1, :] = 1
    else:
        X_occl_sample[i, :, startloc:startloc+1] = 1

X_occl_sample = X_test_sample.view(X_occl_sample.shape[0], -1)
with torch.inference_mode():
    yHat_occlusion = net(X_occl_sample)

fig, axs = plt.subplots(2, 5)
for i in range(5):
    axs[0, i].imshow(X_occl_sample[i, :].view(28, 28), cmap='gray')
    axs[1, i].imshow(yHat_occlusion[i, :].view(28, 28), cmap='gray')
    axs[0, i].set_xticks([]), axs[0, i].set_yticks([])
    axs[1, i].set_xticks([]), axs[1, i].set_yticks([])
plt.show()